...
class FeaturesTest(tf.test.TestCase, parameterized.TestCase):
  ...
  @chex.variants(with_jit=True, without_jit=True)
  def test_custom_gradient_interleaved_captures(self):
    outer_const = tf.constant([1.5, 2.5], dtype=tf.float32)
    ...
    def tf_func(x, y):
      ...
      def inner(u):
        def grad(dy):
          c1 = tf.constant([2.0, 3.0], dtype=tf.float32)
          c2 = tf.constant([4.0, 5.0], dtype=tf.float32)
          return dy * (c1 + x0 * outer_const + x1 * c2 + y)

        return u * 3.0, grad

      return inner(x0) + x1 + y

    np_x = np.array([1.0, 2.0, 3.0, 4.0], dtype=np.float32)
    np_y = np.array([0.5, 1.5], dtype=np.float32)

    with tf.GradientTape() as tape:
      tf_x, tf_y = tf.constant(np_x), tf.constant(np_y)
      tape.watch([tf_x, tf_y])
      tf_outputs = tf_func(tf_x, tf_y)
      tf_loss = tf.reduce_sum(tf_outputs)
    tf_grads = tape.gradient(tf_loss, (tf_x, tf_y))

    with config.override_config("convert_custom_gradient", True):
      jax_func = self.variant(tf2jax.convert_functional(tf_func, np_x, np_y))
      jax_outputs = jax_func(np_x, np_y)
      jax_grads = jax.grad(
          lambda a, b: jnp.sum(jax_func(a, b)), argnums=(0, 1)
      )(np_x, np_y)
  ...
  @chex.variants(with_jit=True, without_jit=True)
  def test_custom_gradient_grad_only_capture(self):
    outer_const = tf.constant([10.0, 20.0], dtype=tf.float32)
    ...
    def tf_func(x, y):
      ...
      def inner(u):
        def grad(dy):
          c1 = tf.constant([2.0, 3.0], dtype=tf.float32)
          c2 = tf.constant([4.0, 5.0], dtype=tf.float32)
          return dy * (c1 + part_sum + outer_const + x1 * c2)
